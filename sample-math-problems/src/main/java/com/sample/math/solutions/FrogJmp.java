 ...
 * jump, at position 10 + 30 + 30 + 30 = 100 Assume that:
 * 
 * X, Y and D are integers within the range [1..1,000,000,000]; X ≤ Y. Complexity:
 * 
 * expected worst-case time complexity is O(1); expected worst-case space complexity is O(1). Copyright 2009–2015 by
...
class FrogJmp {
	...
	public int solution(int X, int Y, int D) {
		Double steps = Math.ceil((Double.valueOf(Y)-Double.valueOf(X))/Double.valueOf(D));
