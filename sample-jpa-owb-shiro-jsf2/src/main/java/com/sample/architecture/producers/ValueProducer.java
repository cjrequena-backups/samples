...
import com.sample.architecture.bindings.Value;
import com.sample.architecture.commons.utils.PropertyResolverUtils;



/**
 * Produces {@link ConfigurationValue} annotated fields. It's responsible for supporting conversion
 * between types (mainly from String to any other type required by the user.)
 * 
 * <p>
 * These producers should not be interested where the fields are read from. It's the
 * {@link PropertyResolverUtils} who is responsible for the configuration loading.
 * </p>
 * 
...
public class ValueProducer {
	...
	@Inject
	PropertyResolverUtils resolver;

	/**
	 * Main producer method - tries to find a property value using following keys:
	 * 
	 * <ol>
	 * <li><code>key</code> property of the {@link ConfigurationValue} annotation (if defined but no
	 * key is found - returns null),</li>
	 * <li>fully qualified field class name, e.g. <code>eu.awaketech.MyBean.myField</code> (if value
	 * is null, go along with the last resort),</li>
	 * <li>field name, e.g. <code>myField</code> for the example above (if the value is null, no can
	 * do - return null)</li>
	 * </ol>
	 * 
	 * @param ip
	...
	public String getStringConfigValue(InjectionPoint ip) {
		...
		String key = ip.getAnnotated().getAnnotation(Value.class).value();
		boolean isKeyDefined = !key.trim().isEmpty();

		boolean valueRequired = ip.getAnnotated().getAnnotation(Value.class).required();
	 ...
	 * 
	 * <p>
	 * Will throw {@link NumberFormatException} if the value cannot be parsed into an
	 * {@link Integer}
	 * </p>
	 * 
	 * @param ip
